    ...
    if (cart.item_count > 0) {
      ...
      $.ajax((_$$ajax = {
        ...
        if (lineID !== null) {
          ...
          var itemTotalhide = $(html).find("[data-line-item=\"".concat(lineID, "\"]")).find('.cart__total-hide');
          ...
          $("[data-line-item=\"".concat(lineID, "\"]")).find('.cart__total-hide').replaceWith(itemTotalhide);
        ...
        var carttotal = $(html).find('.cart__carttotal-container');
        $('.cart__subtotal-container').replaceWith(subtotal);
        $('.cart__carttotal-container').replaceWith(carttotal);

        if (discounts.length > 0 && $('.cart__discounts').length < 1) {
          $('.cart__subtotal-container').before(discounts);
          $('.cart__carttotal-container').before(discounts);
        } else {
        ...
        if (savings.length > 0 && $('.cart__total-savings').length < 1) {
          ...
          $('.cart__carttotal-container').after(savings);
        } else {
