...
const ShippingCalculator = {
  ...
  onShopifyError: function(xhr = null) {
    ...
    if (errors.zip && errors.zip.length > 0) {
      ...
      if (
        ...
        this.$shippingResponseMessage.html(
          `${Shopify.translation.shipping_calculator_zip_code} ${errors.zip}`
        );
      }
    ...
    if (errors.country && errors.country.length > 0) {
      ...
      if (
        errors.country[0].indexOf('is not supported') !== -1 ||
        errors.country[0].indexOf("can't be blank") !== -1
      ) {
        this.$shippingResponseMessage.html(`Country ${errors.country}`);
      }
