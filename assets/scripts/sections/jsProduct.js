Shopify.theme.jsProduct = {
  init: function ($section) {
    ...
    // Selectors
    const $mainProductGallery = $section.find('.product-gallery__main');
    const $stickyElement = $section.find('.sticky-product-scroll');
    const $productTabBar = 'product__tab--bar';
    ...
    $('.product-tech_specs h1 a').on('click', function(e){
      e.preventDefault();
      if ($(this).hasClass('active')) {
        $('.product-tech_specs .tech__specs--content').slideUp();
      } else {
        $('.product-tech_specs .tech__specs--content').slideDown();
      }
  ...
  enableStickyTabBar($el) {
    ...
    var stop = (element.offsetTop + 130);
    window.onscroll = function(e) {
      var scrollTop = (window.pageYOffset !== undefined) ? window.pageYOffset : (document.documentElement || document.body.parentNode || document.body).scrollTop;

      if (scrollTop >= stop) {
          element.classList.add('sticky');
          element.nextElementSibling.classList.add('sticky-enabled');
      } else {
  ...
  disableSlideshow($section) {
    var $slider = $section.find('.flickity-enabled');
    $slider.flickity('destroy');
  },
  enableFeaturedSlideshow($el) {
  ...
  enableSlideshow($el) {
    const $thumbnailProductGallery = $el.parents('.product-gallery').find('.product-gallery__thumbnails');
    ...
    if (this.enable_thumbnail_slider === true && this.product_images_amount > 1) {
      ...
      if(this.thumbnail_position == 'right-thumbnails' || this.thumbnail_position == 'left-thumbnails') {
        ...
        $el.on( 'select.flickity', function() {
          ...
          var $selected = $thumbnailNavCells.eq( flkty.selectedIndex ).addClass('is-nav-selected');
    ...
    const $galleryThumbnail = $thumbnailProductGallery.find('.product-gallery__thumbnail');
  ...
  scrollSelectedImage: function(variant) {
    let headerHeight = 0;
    ...
    // Add values
    let totalHeight = headerHeight + announceHeight;
