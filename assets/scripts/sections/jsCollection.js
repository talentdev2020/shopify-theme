Shopify.theme.jsCollection = {
  ...
  updateView: function(filterURL) {
    $.ajax({
      ...
      const $collectionMain = $('.collection__main');
      ...
      $collectionMatrix.removeClass('loading-in-progress');
  ...
  multiTagFilter: {
    ...
    showSelectedFilter: function ($el) {
      ...
      if ($sidebarToggleBlock.length) {
        let $toggleBtn = $sidebarToggleBlock.find('[data-sidebar-block__toggle-icon="closed"]');
