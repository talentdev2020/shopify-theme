Shopify.theme.jsMegaMenu = {
  init: function($section) {
    ...
    const $parentMobileElement = $('.mobile-menu [data-navlink-handle="' + parentLink + '"]');
    ...
    $parentMobileElement.find('.mega-menu__section').remove();

    // Mobile
    if(!$parentMobileElement.next('ul').length) {

      // Assign class to display dropdown arrow
      $parentMobileElement.find('a').addClass('mobile-menu-link');
      ...
      $('.has-no-menu').each(function(){
        $(this)
          .parent('li')
          .wrap('<div class="has-submenu"></div>');
      });

      $('<span class="close-dropdown"></span>')
        .insertAfter($parentMobileElement.find('a'));

      $('<ul class="mobile-menu__submenu animated slideInDown"></ul>')
        .insertAfter($parentMobileElement);

      // Insert mega menu
      let $parentList = $parentMobileElement.next('ul');
    ...
    $('.mega-menu__section, .navbar, .header__brand').on('mouseleave', function(){
